...
public abstract class RenderTarget2DControl : Control
{
    ...
    /// <summary>
    ///     Draws the cached render target onto the screen and requests new data after the <see cref="DrawInterval" /> is
    ///     passed.
    /// </summary>
        ...
        if (this._renderTargetLock.IsFree())
        {
            using (this._renderTargetLock.Lock())
            {
                if (this._renderTarget != null)
                {
                    ...
                    spriteBatch.DrawOnCtrl(this, this._renderTarget, bounds, Color.White);
                    spriteBatch.End();
                }
            }
        }

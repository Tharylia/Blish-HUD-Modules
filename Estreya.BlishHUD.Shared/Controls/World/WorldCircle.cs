namespace Estreya.BlishHUD.Shared.Controls.World
{
    ...
    {
        public WorldCircle(Vector3 position, float radius, int tessellation = 50) : this(position, radius, Color.White, tessellation) { }

        public WorldCircle(Vector3 position, float radius, Color color, int tessellation = 50) : base(position, Enumerable.Range(0, tessellation + 1).SelectWithIndex((t, index, sourceList) =>
        {

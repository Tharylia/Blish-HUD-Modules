...
{
    private readonly List<ParsedEvtcLog> _logs = new List<ParsedEvtcLog>();
    ...
    {
        return new EvtcParser(new EvtcParserSettings(false, false, false, false, false, 2000), this._parserAPIController);
        ;
    }
    ...
    protected override async Task LoadAsync()
    {
        ...
        string gamePath = @"C:\Program Files\Guild Wars 2\Gw2-64.exe";
    ...
    private async Task LoadLogs()
    {
        ...
        string path = Path.Combine(this.DirectoriesManager.GetFullDirectoryPath(directoryName), "cache");

        IProgress<string> progress = new Progress<string>(status => this.ReportLoading("logs", $"Loading logs: {status}"));
        ParserController operation = new ParserController(null);
        int loadedLogs = 0;
        try
        {
            List<Task<ParsedEvtcLog>> tasks = new List<Task<ParsedEvtcLog>>();

            Parallel.ForEach(files.ChunkBy(25), new ParallelOptions { MaxDegreeOfParallelism = 4 }, fileChunk =>
            {
                try
                {
                    foreach (string file in fileChunk)
                    {
                        ParsedEvtcLog log = this.GetParser().ParseLog(operation, new FileInfo(file), out ParsingFailureReason parsingFailureReason, true);
    ...
    private void CreateManagerWindow()
    {
        ...
        this._managerWindow.Tabs.Add(new Tab(this.IconService.GetIcon("156736.png"),
            () => new GeneralSettingsView(
                this.ModuleSettings, this.Gw2ApiManager, this.IconService, this.TranslationService, this.SettingEventService, GameService.Content.DefaultFont16) { DefaultColor = this.ModuleSettings.DefaultGW2Color }
            , "General"));
    }
